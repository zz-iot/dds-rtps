...
// implementations live in dds_impl.zig, generated by zidl from srcZig/shape.idl.
//
//   pub const ShapeType = struct {
//       color: <vendor-defined owned string type>,
//       x: i32,
//       y: i32,
//       shapesize: i32,
...
//
//       // Set color from a []const u8 slice (makes an internal copy).
//       pub fn setColor(self: *ShapeType, c: []const u8) void;
//   };
//   ShapeType.color is readable as []const u8 via .color.slice().
//
...
//
//   pub fn deserializeShapeKey(payload: []const u8) <vendor color type>;
//     Extract the color key from any ShapeType CDR payload (full or key-only).
//     Call .slice() on the result to obtain []const u8.
//
